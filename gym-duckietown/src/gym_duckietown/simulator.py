...
from .graphics import (
    ...
    load_texture,
    Texture,
    bezier_draw_points_curve,##new riza
    bezier_draw_line,
    draw_point,
    get_linear_bezier,
    rotate_translate,
    compute_dist,
    inverse_rotate_translate,###end riza
)
...
class Simulator(gym.Env):
    ...
    def __init__(
        ...
        evaluate=False,##############riza new
        draw_DDPG_features=True,############riza new
    ):
        """
        ...
        :param dataset_gen: (recordpos, loadpos)
        :param seed:
        ...
        if dataset_gen == (True, False):
            self.record_start_tile = True
            self.load_start_tile = False
            self.record_start_pose = True
            self.load_start_pose = False
        
        if dataset_gen == (False, True):
            self.record_start_tile = False
            self.load_start_tile = True
            self.record_start_pose = False
            self.load_start_pose = True

        if dataset_gen == (False, False):
            self.record_start_tile = False
            self.load_start_tile = False
            self.record_start_pose = False
            self.load_start_pose = False
        ...
        # Flag for evaluation over a map ###########################new riza
        self.evaluate = evaluate
        # required for evalutaion
        self.env_count = 0
        # Flag for drawing features
        self.draw_DDPG_features = draw_DDPG_features################end riza
    ...
    def reset(self, segment: bool = False):
        ...
        if self.domain_rand:
            ...
            #light_pos = [0.0, 3.0, 0.0, 1.0]   LUCI ORIGINALI

        # DIM = 0.0
        ambient = np.array([0.50 * DIM, 0.50 * DIM, 0.50 * DIM, 1])

        #ambient = self._perturb(ambient, 0.3) LUCI ORIGINALI
        ambient = self._perturb(ambient, 0)
        ...
        if self.load_start_tile: #FOR DATASET GENERATION 
            ###################-------read
            data = {}
            file1 = open('tilesRecordin.jsonl', 'r')

            data = file1.readline()
            data2 = file1.readline()
            newtile = json.loads(data)
        
            #print(np.asarray(newtile['coords']))
            newtile['coords']= tuple(newtile['coords'])
            newtile['curves']=np.asarray(newtile['curves'])
            print("CURVES: --------------------------------", newtile['curves'])
            print("COORDINATES: --------------------------------", newtile['coords'])
            #to take effect
            #tile['coords']= newtile['coords']
            file1.close()
            
        # tile['kind']= newtile['kind']
            

            tile = self._get_tile(newtile['coords'][0], newtile['coords'][1])
            #tile['angle']= newtile['angle']
            #tile['drivable']= newtile['drivable']
            #tile['curves']= newtile['curves']

      # If the map specifies a starting tile
        elif self.user_tile_start:
        ...
        ###################-------write
        if self.record_start_tile:
        
            #we remove texture and color we are just intrested in the position and angle
            savetile = tile.copy()
            savetile.pop('texture', None)
            savetile.pop('color', None)
        
            #tile['texture'] = <gym_duckietown.graphics.Texture object at 0x7fc784dc88d0>
            
            #to support np array in json
            ...
            with open("tilesRecordin.jsonl", "a") as outfile:
                json.dump(savetile, outfile, cls=NpEncoder)
                outfile.write('\n')
        ...
        if self.load_start_pose:
            data = {}
            file1 = open('positionRecordin.jsonl', 'r')

            data = file1.readline()
            newpos = json.loads(data)
        
            #print(np.asarray(newtile['coords']))
           
            newpos['pos']=np.asarray(newpos['pos'])
            print("POS: --------------------------------", newpos['pos'])
            print("ANGLE: --------------------------------", newpos['angle'])

            propose_pos = newpos['pos']
            propose_angle = newpos['angle']
            file1.close()
        ...
        #     propose_pos = np.array([1., 0., 2.7])
            ...
            for _ in range(MAX_SPAWN_ATTEMPTS):
                i, j = tile["coords"]
                # beginning of riza Choose a random position on this tile added 0.2 and 0.8 to avoid  starting at the edges of the tile
                ...
                propose_pos = np.array([x, 0, z])###########end of riza
        ...
        if self.record_start_pose:
        
            #we remove texture and color we are just intrested in the position and angle
          
            d = {}
            d['pos']=self.cur_pos
            d['angle']=self.cur_angle
            ...
            with open("positionRecordin.jsonl", "a") as outfile2:
                json.dump(d, outfile2, cls=NpEncoder)
                outfile2.write('\n')
        ...
        logger.info(f"Starting at {self.cur_pos} {self.cur_angle} {self.cur_tile}")
        ...
        self.last_state = np.zeros((1, 189))###end of riza
    ...
    def _get_curve(self, i, j):
        ...
        elif kind == "curve_right":#########riza values were wrong
            pts = (
                np.array(
                    [
                        [
    ...
    def step(self, action: np.ndarray):
        ...
        action = np.clip(action, 0, 1)       # action = np.clip(action, -1, 1)#end of riza
    ...
    def _compute_done_reward(self) -> DoneRewardInfo:
        ...
        # @riza :If duckie is too far from center line (on the other lane, etc.)
            ...
            reward = REWARD_INVALID_POSE
            done_code = 'far from center line'######end of riza
    ...
    def _render_img(
        ...
        def loadskybox():
            image_data = BytesIO(requests.get("https://images.unsplash.com/photo-1664575196412-ed801e8333a1?ixlib=rb-1.2.1&ixid=MnwxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=2071&q=80").content)
            im1 = Image.open(image_data).rotate(180).transpose(Image.FLIP_LEFT_RIGHT).resize((512,512))
        ...
        '''    #background image
        bg = pyglet.resource.image("src/gym_duckietown/background.jpg")
        ...
        if not segment:
            ...
            #self.tri_vlist.draw(gl.GL_TRIANGLES) RUMORE MODIFICA
            gl.glPopMatrix()
        ...
        #------------------------------------------------------MOD (WAS POS ANG -> SELF.CUR_POS, SELF.CUR_ANG))
        if self.draw_bbox:
            corners = get_agent_corners(self.cur_pos, self.cur_angle)
            #import random
        ...
        # @riza: Uncomment to draw the line b/w closest curve point & center of robot (perpendicular line)
           #self.dist_centerline_curve()#end of riza
...
def get_dir_line(angle, pos):
    # Get directory vector
    x_, _, y_ = get_dir_vec(angle)
    # Transform dir_vector to unit vector
